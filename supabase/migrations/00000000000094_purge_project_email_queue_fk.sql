-- purge_project (migration 21) predates inbound_email_queue (migration 90).
-- proposed_project_id/resolved_project_id use the default NO ACTION FK behavior,
-- so a project with queue rows still pointing at it fails the hard-delete with
-- a foreign key violation. Nullify them first, same pattern as credit_ledger.
