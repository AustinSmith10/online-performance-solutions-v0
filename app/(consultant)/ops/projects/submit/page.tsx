...
export default async function ConsultantSubmitPage({
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-10">
        <div className="mb-8">
          ...
          <h1 className="mt-2 text-xl font-semibold text-zinc-900">Upload documents for a client</h1>
  ...
  // ── Step 2: pick stakeholder account within the client ───────────────────
  if (!clientId) {
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-10">
        <div className="mb-8">
          <Link href="/ops/projects/submit" className="text-sm text-zinc-500 hover:text-zinc-700">
            ← Change client
          </Link>
          <h1 className="mt-2 text-xl font-semibold text-zinc-900">
            Upload documents — {selectedOrg.name}
          </h1>
          <p className="mt-1 text-sm text-zinc-500">
            Select the stakeholder account this upload is being made on behalf of.
          </p>
        </div>
        <div className="rounded-lg border border-zinc-200 bg-white p-6">
  ...
  // ── Step 3: validate stakeholder belongs to client, then show upload form ─
  ...
  if (activeTemplates.length === 0) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-10">
        <div className="mb-8">
          <Link
            href={`/ops/projects/submit?client_id=${orgId}`}
        ...
        <div className="rounded-lg border border-zinc-200 bg-white p-8 text-center">
          ...
          <p className="mt-1 text-sm text-zinc-500">
            {selectedOrg.name} has no active templates. An admin needs to activate one before you can submit.
          </p>
        </div>
      </div>
    );
    ...
    <div className="mx-auto max-w-2xl px-4 py-10">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-zinc-500">
          ...
          ({client.email}). The extracted fields and files will be attributed to you as the uploader.
        </p>
      </div>
      <SubmissionForm
        ...
        startOverHref={`/ops/projects/submit?client_id=${orgId}`}
      />
    </div>
  );
}
