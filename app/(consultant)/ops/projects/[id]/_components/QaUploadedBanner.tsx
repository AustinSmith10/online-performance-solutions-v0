...
export function QaUploadedBanner({ cleanUrl }: { cleanUrl: string }) {
  ...
  useEffect(() => {
    const el = document.getElementById("qa-pbdb-row") ?? document.getElementById("pbdb-section");
    if (el) {
      el.scrollIntoView({ behavior: "smooth", block: "center" });
      el.classList.add("ring-2", "ring-green-400", "ring-offset-2", "rounded-lg");
      setTimeout(() => el.classList.remove("ring-2", "ring-green-400", "ring-offset-2", "rounded-lg"), 2500);
    }
    pbdbRef.current = el;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black/30">
      <div className="mx-4 w-full max-w-sm rounded-xl border border-zinc-200 bg-white p-8 shadow-xl text-center">
        ...
        <p className="text-base font-semibold text-zinc-900">PBDB uploaded</p>
        <p className="mt-2 text-sm text-zinc-500">
          Your QA&rsquo;d PBDB has been saved and dispatched to stakeholders for approval.
        </p>
        <button
          type="button"
          onClick={() => {
